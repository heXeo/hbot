...
function injectSecretKey(serviceApiContent: any, secretKey: string) {
  ...
  serviceApiContent.TaskTemplate.ContainerSpec.Env = (env || []).concat(
...
function injectStackLabels(serviceApiContent: any, stackName: string) {
  ...
  const imageLabel =
    !imageParts[1] || imageParts[1].toLowerCase() === 'latest'
      ? imageParts[0]
...
function clearStackLabels(serviceApiContent: any) {
...
export default class Swarm {
  ...
  async deploy(
    ...
    return Promise.all(
      apiSpecs.map(async (apiSpec: any) => {
        const serviceName = isStack
          ? `${definitionName}_${apiSpec.Name}`
          : definitionName
        ...
        if (force) {
          ...
          apiSpec.TaskTemplate.ForceUpdate =
            (previousSpec.TaskTemplate.ForceUpdate || 0) + 1
        }
  ...
  async listStacks() {
    ...
    return _
      .uniqBy(stacksServices, `Spec.Labels['${stackNamespaceLabel}']`)
  ...
  async checkUnreferencedServices(definitionName: string, apiSpecs: any[]) {
    const currentServices = await this.searchServicesByDefinition(
      definitionName
    )

    return currentServices.filter(
      (currentService: any) =>
        apiSpecs.findIndex((apiSpec: any) => {
          return (
            currentService.Spec.Name === `${definitionName}_${apiSpec.Name}` ||
  ...
  async getServiceLogs(name: string) {
    ...
    return tasks.reduce(async (promise, task) => {
      ...
      try {
        const taskLogs = await this.api.get(
          `/containers/${task.Status.ContainerStatus.ContainerID}/logs`,
          {
            query: {
              stdout: true,
              stderr: true,
